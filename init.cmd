...
Echo.
Echo.     [2] Verificar se seu Windows está ativo
echo. 
...
mode con cols=68 lines=13
	 echo.
     echo  Verificando o status de ativação...
	 echo.
	 echo  Windows %osedition% %vera% está permanentemente ativado.
     echo  Ativação não necessária.
	 echo.
...
cd /d "%~dp0"
cls
echo "LICENÇA DIGITAL DO WINDOWS [Windows %osedition% %vera%]"
echo:
if [%osedition%] == [Cloud] (
	set "edition=Cloud"
...
echo "Licença Digital do Windows [Windows %osedition% %vera%]"
echo:
echo Instalando a chave de produto padrão para Windows %edition% %vera%...
echo:
...
:Activated
	 echo O Windows detectado %osedition% %vera% está permanentemente ativado.
     echo A ativação não é necessária.
	 echo.
     echo Pressione qualquer tecla para continuar...
     pause >nul
	 mode con cols=60 lines=13
...
timeout /t 3 >nul 2>&1
echo:
echo O arquivo GenuineTicket.XML está sendo instalado para Windows %edition% %vera%...
echo:
...
echo. :=======================================================:
Echo.
Echo.     [4] Inserir chave de produto padrão
echo.
Echo.     [5] Ver Repositório
Echo.       
Echo.     [6] Vá para o menu principal
Echo.
echo. :=======================================================:
